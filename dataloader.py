...
def get_pickle_paths():
    return dict(
        ...
        file_train_categories_train_base_phase = os.path.join(
            _MINI_IMAGENET_DATASET_DIR,
            'miniImageNet_category_split_train_phase_train_base.pickle'),
        file_train_categories_train_base2_phase = os.path.join(
            _MINI_IMAGENET_DATASET_DIR,
            'miniImageNet_category_split_train_phase_train_base2.pickle'),
        file_train_categories_val_phase = os.path.join(
            _MINI_IMAGENET_DATASET_DIR,
            'miniImageNet_category_split_train_phase_val.pickle'),
        file_train_categories_val_base_phase = os.path.join(
            _MINI_IMAGENET_DATASET_DIR,
            'miniImageNet_category_split_train_phase_val_base.pickle'),
        file_train_categories_val_base2_phase = os.path.join(
            _MINI_IMAGENET_DATASET_DIR,
            'miniImageNet_category_split_train_phase_val_base2.pickle'),
...
class MiniImageNet(data.Dataset):
    @classmethod
    def get_pickle_paths(cls):
        return get_pickle_paths()

    def __init__(self, phase='train', do_not_use_random_transf=False, get_pickle_paths=None):

        assert phase in ['train', 'train_base', 'train_base2', 'val', 'val_base', 'val_base2', 'test']
        self.phase = phase
        ...
        file_test_categories_test_phase = os.path.join(
            _MINI_IMAGENET_DATASET_DIR,
            'miniImageNet_category_split_test.pickle')
        if get_pickle_paths is not None:
            pickle_paths = get_pickle_paths()
            file_train_categories_train_phase = pickle_paths['file_train_categories_train_phase']
            file_train_categories_train_base_phase = pickle_paths['file_train_categories_train_base_phase']
            file_train_categories_val_phase = pickle_paths['file_train_categories_val_phase']
            file_train_categories_val_base_phase = pickle_paths['file_train_categories_val_base_phase']
            file_train_categories_test_phase = pickle_paths['file_train_categories_test_phase']
            file_val_categories_val_phase = pickle_paths['file_val_categories_val_phase']
            file_test_categories_test_phase = pickle_paths['file_test_categories_test_phase']
            self.use_precomputed_features = True
        else:
            self.use_precomputed_features = False

        if self.phase.startswith('train'):
            # During training phase we only load the training phase images
            # of the training categories (aka base categories).
            if self.phase == 'train':
                data_train = load_data(file_train_categories_train_phase)
            elif self.phase == 'train_base':
                data_train = load_data(file_train_categories_train_base_phase)
            elif self.phase == 'train_base2':
                data_train = load_data(file_train_categories_train_base2_phase)
            else:
                assert False
        ...
        elif self.phase.startswith('val') or self.phase == 'test':
            if self.phase=='test':
            ...
            elif self.phase.startswith('val'):
                # load data that will be used for evaluating the recognition
                # accuracy of the base categories.
                if self.phase=='val':
                    data_base = load_data(file_train_categories_val_phase)
                elif self.phase=='val_base':
                    data_base = load_data(file_train_categories_val_base_phase)
                elif self.phase=='val_base2':
                    data_base = load_data(file_train_categories_val_base2_phase)
                else:
                    assert False
                # load data that will be use for evaluating the few-shot recogniton
                # accuracy on the novel categories.
        ...
        if (self.phase=='test' or self.phase=='val' or self.phase.startswith('val_base') ) or (do_not_use_random_transf==True):
    ...
    def get_all_images_of_class(self, cls_id):
        imgs = []
        ids = []
        for i in range(len(self.data)):
            if self.labels[i] == cls_id:
...
class FewShotDataloader():
    def __init__(self,
        ...
        max_possible_nKnovel = (self.dataset.num_cats_base if self.phase in ['train', 'train_base', 'train_base2']
        ...
        self.is_eval_mode = (self.phase=='test') or (self.phase=='val') or (self.phase.startswith('val_base'))
    ...
    def get_iterator_with_global_ids_and_labels(self, epoch=0):
        rand_seed = epoch
        random.seed(rand_seed)
        np.random.seed(rand_seed)
        def load_function(iter_idx):
            ...
            if len(Exemplars) > 0:
                Xe, Ye = self.createExamplesTensorData(Exemplars)
                return (Xe, Ye, torch.tensor([i for i, _ in Exemplars]), torch.tensor([self.dataset.labels[i] for i, _ in Exemplars]),
                        Xt, Yt, torch.tensor([i for i, _ in Test]), torch.tensor([self.dataset.labels[i] for i, _ in Test]), Kall, nKbase)
            else:
                return Xt, Yt, torch.tensor([i for i, _ in Test]), torch.tensor([self.dataset.labels[i] for i, _ in Test]), Kall, nKbase
