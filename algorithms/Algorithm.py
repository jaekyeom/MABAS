...
class Algorithm():
    def __init__(self, opt):
        ...
        self.logger.info('Environment variables %s' % os.environ)
        self.opt = opt
    ...
    def set_tensorboard_logger(self):
        tb_dir = os.path.join(self.exp_dir, 'tb')
        os.makedirs(tb_dir)
        ...
        if pretrained_model['network'].keys() == network.state_dict().keys():
            ...
            for pname, param in network.named_parameters():
                if pname in pretrained_model['network']:
                    ...
                    self.logger.info('WARNING: parameter {} not loaded'.format(pname))
            for bname, buffer in network.named_buffers():
                if bname in pretrained_model['network']:
                    self.logger.info('==> Copying buffer %s from file %s' %
                                     (bname, pretrained_path))
                    buffer.data.copy_(pretrained_model['network'][bname])
                else:
                    self.logger.info('WARNING: buffer {} not loaded'.format(bname))
    ...
    def delete_checkpoint(self, epoch, suffix=''):
        for key, net in self.networks.items():
            ...
            if False:
                self.logger.info('NOT deleting old checkpoint {}'.format(filename_net))
            else:
                if os.path.isfile(filename_net): os.remove(filename_net)

            filename_optim = self._get_optim_checkpoint_filename(key, epoch)+suffix
            if False:
                self.logger.info('NOT deleting old checkpoint {}'.format(filename_optim))
            else:
    ...
    def solve(self, data_loader_train, data_loader_test=None):
        ...
        for self.curr_epoch in xrange(start_epoch, self.max_num_epochs):
            ...
            for k, v in train_stats.items():
                self.tb_writer.add_scalar('train/avg_{}'.format(k),
                                          v,
                                          self.curr_epoch)
            ...
            for k, n in self.networks.items():
                self.logger.info('FINGERPRINT of {}: {}'.format(
                        k, utils.get_network_fingerprint(n)))
    ...
    def evaluate(self, dloader, is_test=False):
        ...
        avg_eval_stats = eval_stats.average()

        for k, v in avg_eval_stats.items():
            self.tb_writer.add_scalar('{}/avg_{}'.format('test' if is_test else 'eval', k),
                                      v,
                                      self.curr_epoch)
            self.tb_writer.flush()
