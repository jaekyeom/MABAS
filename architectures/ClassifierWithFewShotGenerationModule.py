...
class Classifier(nn.Module):
    def __init__(self, opt):
        ...
        base_label_ids = opt.get('base_label_ids', list(range(nKall)))
        assert len(base_label_ids) == nKall
        self.base_id_map = np.zeros(np.max(base_label_ids) + 1, dtype=int)
        self.base_id_map.fill(-1)
        for i, v in enumerate(base_label_ids):
            self.base_id_map[v] = i
        self.base_id_map = torch.tensor(self.base_id_map)
    ...
    def get_classification_weights(
        ...
        weight_base = self.weight_base[self.base_id_map[Kbase_ids.view(-1)]]
        ...
        if self.weight_generator_type=='none':
            ...
            weight_novel_att = self.attblock(
                features_train, labels_train, weight_base_tmp, self.base_id_map[Kbase_ids.view(-1)])
    ...
    def apply_classification_weights(self, features, cls_weights, out_data=dict()):
        ...
        if self.classifier_type=='cosine':
            if features is not None:
                ...
                if 'normalized_features' in out_data:
                    out_data['normalized_features'] = features
            cls_weights = F.normalize(
                cls_weights, p=2, dim=cls_weights.dim()-1, eps=1e-12)
            if 'normalized_cls_weights' in out_data:
                out_data['normalized_cls_weights'] = cls_weights

        if features is None:
            return None
        cls_scores = torch.baddbmm(1.0,
            self.bias.view(1, 1, 1), 1.0, features, cls_weights.transpose(1,2))
        if 'unscaled_scores' in out_data:
            out_data['unscaled_scores'] = cls_scores
        cls_scores = self.scale_cls * cls_scores
        return cls_scores


    def forward(self, features_test, Kbase_ids, features_train=None, labels_train=None, out_data=dict()):
        ...
        if 'unnormalized_cls_weights' in out_data:
            out_data['unnormalized_cls_weights'] = cls_weights
        cls_scores = self.apply_classification_weights(
            features_test, cls_weights, out_data=out_data)
        return cls_scores
